...
for arch in ${architectures[*]}; do
  for mthd in ${methods[*]}; do
    for seed in ${seeds[*]}; do
      python test_time.py --cfg "cfgs/imagenet_c/${mthd}.yaml" MODEL.ARCH $arch RNG_SEED $seed CORRUPTION.NUM_EX 50000 $options CORRUPTION.TYPE "['gaussian_noise', 'none',
       'shot_noise', 'none', 'impulse_noise', 'none', 'defocus_blur', 'none', 'glass_blur', 'none',
       'motion_blur', 'none', 'zoom_blur', 'none', 'snow', 'none', 'frost', 'none', 'fog', 'none',
       'brightness', 'none', 'contrast', 'none', 'elastic_transform', 'none', 'pixelate', 'none',
